        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted">Platform Owner</p>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-6 sm:px-6 sm:py-8">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted">
            Cross-platform account and church analytics. This dashboard is separate from church-level Super Admin access.
          </p>
        </div>

        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Metric label="Registered churches" value={totals.churches} note={`${totals.churchesLast7Days} in the last 7 days`} />
          <Metric label="User accounts" value={totals.users} note={`${totals.activeUsers} active`} />
          <Metric label="Awaiting church setup" value={totals.awaitingChurchSetup} note="Signed up, no church yet" />
          <Metric label="Programmes recorded" value={totals.programmes} note={`${totals.branches} active branches`} />
        </section>
        ...
        <section>
          <div className="mb-3 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-muted">Latest organisations registered on Church OMS.</p>
            </div>
          </div>
                  ...
                  {data.churches.map((church) => (
                    <tr key={church.id}>
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-muted">{church.currency_code} · {church.timezone}</p>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">{church.active_user_count}/{church.user_count} active</td>
                      <td className="px-4 py-3">{church.branch_count}</td>
                      <td className="px-4 py-3">{church.programme_count}</td>
                      <td className="px-4 py-3 text-muted">{formatDate(church.created_at)}</td>
                    </tr>
                  ))}
                  {data.churches.length === 0 && (
                    <tr><td colSpan={6} className="px-4 py-8 text-center text-muted">No churches registered yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <section>
          <div className="mb-3">
            ...
            <p className="text-sm text-muted">The newest people creating or joining Church OMS accounts.</p>
          </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-xs text-muted">
                      {account.church_name ?? "Awaiting church setup"} · {formatDateTime(account.created_at)}
                    </p>
                  </div>
                  <span className={`rounded-full px-2.5 py-1 text-xs font-medium ${account.active ? "bg-success/10 text-success" : "bg-surface-border text-muted"}`}>
